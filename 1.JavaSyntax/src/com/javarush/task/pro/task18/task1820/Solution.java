...
public class Solution {
    ...
    public static Optional<Car> getMostExpensiveCar(Stream<Car> cars) {
        ...
        Stream<Car> max =  cars;
        Optional<Car> optionalCar = max.max((s1, s2) -> s1.getPrice() - s2.getPrice());
        return optionalCar;
    }

    public static Optional<Car> getMoreExpensiveCar(Stream<Car> cars, Car mostExpensiveCar) {
        Optional<Car> carExpThenThere = cars.filter((x) -> x.getPrice() > mostExpensiveCar.getPrice()).findFirst();
        return carExpThenThere;
        }
}
